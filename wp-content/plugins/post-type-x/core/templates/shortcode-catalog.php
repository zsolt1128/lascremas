...
//add_filter( 'ic_catalog_body_class', array( $this, 'catalog_query' ), 98 );
	...
	function default_message() {
		...
		if ( !empty( $page_url ) ) {
			implecode_info( '<p>' . sprintf( __( 'Currently %sanother page%s is set to show main product listing. Would you like to show product listing here instead?', 'post-type-x' ), '<a href="' . $page_url . '">', '</a>' ) . '</p><button  type="button" class="button assign-listing-button">' . __( 'Yes', 'post-type-x' ) . '</button><div class="ic_spinner"></div><p style="font-size: 0.8em">* ' . sprintf( __( 'Please remove the %s shortcode to disable this info.', 'post-type-x' ), '[show_product_catalog]' ) . '</p>' );
			echo "<script>jQuery('.assign-listing-button').click(function() {" . $this->assing_listing_script( $page_id ) . "});</script>";
		} else {
			if ( !empty( $page_id ) ) {
				echo __( 'Please wait... we are preparing your product listing page.', 'post-type-x' ) . '<div class="ic_spinner"></div>';
