		...
		if ( is_ic_integration_wizard_page() ) {
			...
			if ( !isset( $_GET[ 'test_advanced' ] ) ) {
				if ( is_ic_shortcode_integration() ) {
					...
					$box_content .= '<p>' . __( 'In Simple Mode the product listing, product search and category pages are disabled (please read this Sample Product Page to fully understand the difference).', 'post-type-x' ) . '</p>';
				...
				if ( is_ic_shortcode_integration() ) {
					$box_content .= '<p class="initial-description">' . __( 'Would you like to remove the shortcode integration and try the configuration wizard?', 'post-type-x' ) . '</p><p><strong>' . sprintf( __( "If something goes wrong you will have to add the %s shortcode to the product listing again.", 'post-type-x' ), '[[show_product_catalog]]' ) . '</strong></p><p><strong>' . __( 'Do it only if the catalog layout doesn\'t match your theme or if some unwanted elements from the theme are displayed on catalog pages.', 'post-type-x' ) . '</strong></p>';
					$box_content .= '<p class="wp-core-ui" style="margin-top: 20px;"><a href="' . add_query_arg( 'remove_shortcode_integration', '1' ) . '" class="button-primary">' . __( 'Remove the Shortcode and Start the Wizard', 'post-type-x' ) . '</a></p>';
				} else {
...
//$box_content .= '<p>' . __( 'Please use the buttons below to let the script know if the Automatic Advanced Integration is done right.', 'post-type-x' ) . '</p>';
					...
					$box_content			 .= implecode_settings_number( __( 'Padding', 'post-type-x' ), 'container_padding', $integration_settings[ 'container_padding' ], 'px', 0, null, __( 'Increase this number to make a space also on the top and bottom of the container. This is useful also if you are planning to enable the sidebar in next step.', 'post-type-x' ), 0 );
					$box_content			 .= '</tbody>';
	...
	margin-left: -23px;}</style>';
					$box_content .= '<p class="wp-core-ui"><span class="ic_spinner"></span><a href="' . esc_url( add_query_arg( 'test_advanced', 'ok' ) ) . '" class="button-primary integration-ok">' . __( 'It\'s Fine', 'post-type-x' ) . '</a><a href="' . esc_url( add_query_arg( 'test_advanced', 'bad' ) ) . '" class="button-secondary">' . __( 'It\'s Broken', 'post-type-x' ) . '</a> <button class="button-secondary show_third switch_section">' . __( 'Go Back', 'post-type-x' ) . '</button></p>';
					$box_content .= '</div>';
				...
				$box_content .= '<li>' . sprintf( __( 'Use %s shortcode on your product listing page.', 'post-type-x' ), '[[show_product_catalog]]' ) . '</li>';
				...
				$box_content .= '<p>' . sprintf( __( 'If you are a developer or would like to have full control on the product pages templates make sure to see the %stemplates docs%s.', 'post-type-x' ), '<a href="https://implecode.com/docs/ecommerce-product-catalog/product-page-template/">', '</a>' ) . '</p>';
