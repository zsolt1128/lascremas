...
function ic_get_search_page_title() {
	$page_title = __( 'Search Results for:', 'post-type-x' ) . ' <span class="ic-search-keyword">' . strval( $_GET[ 's' ] ) . '</span>';
	return $page_title;
}
...
function ic_product_listing_products( $archive_template, $multiple_settings ) {
	...
	if ( (is_tax() || is_search() || !is_ic_only_main_cats() || is_product_filters_active() || is_ic_ajax()) && more_products() ) {
		...
		$notfound_filters_text = sprintf( __( 'Sorry, but nothing matched your search terms. Please try again with some different options or %sreset filters%s.', 'post-type-x' ), '<a href="' . get_filters_bar_reset_url() . '">', '</a>' );
