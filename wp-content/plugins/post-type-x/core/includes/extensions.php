...
function product_extensions() {
	...
	<div id="implecode_settings" class="wrap">
		<h2><?php echo sprintf( __( 'Extensions for %s', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) ?></h2>
...
function ic_epc_submenu_extensions_info() {
	if ( !function_exists( 'start_implecode_updater' ) ) {
		echo '<span class="extensions-promo-box">' . sprintf( __( 'Add free & premium features %shere%s.', 'post-type-x' ), '<a href="' . admin_url( 'edit.php?post_type=al_product&page=extensions.php' ) . '">', '</a>' ) . '</span>';
	}
}
