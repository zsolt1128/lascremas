...
class ic_cat_activation_wizard extends ic_activation_wizard {
	...
	function activation_notices() {
		...
		if ( is_ic_activation_notice() && !$this->get_notice_status( 'notice-ic-catalog-activation' ) ) {
			...
			if ( !empty( $not_complete ) ) {
				$this->box_paragraph( __( 'You will be able to change your choice later in catalog setting or by reactivating the plugin.', 'post-type-x' ), true );
			}
			$this->wizard_box();
	...
	function shipping_choice_response() {
		$response[ 'one' ]		 = __( 'Shipping Enabled', 'post-type-x' );
		$response[ 'next_one' ]	 = 'shipping-on';
		$response[ 'two' ]		 = __( 'Shipping Disabled', 'post-type-x' );
		$response[ 'next_two' ]	 = 'shipping-off';
		return $response;
	}

	function price_choice_response() {
		$response[ 'one' ]		 = __( 'Price Enabled', 'post-type-x' );
		$response[ 'next_one' ]	 = 'price-on';
		$response[ 'two' ]		 = __( 'Price Disabled', 'post-type-x' );
