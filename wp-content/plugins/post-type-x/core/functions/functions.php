...
function get_product_catalog_page_title() {
	if ( is_ic_taxonomy_page() ) {
		...
		$title = __( 'Search Results for:', 'post-type-x' ) . ' <span class="ic-search-keyword">' . strval( $_GET[ 's' ] ) . '</span>';
	...
	if ( is_product_sort_bar_active() || is_ic_ajax() ) {
		if ( is_active_sidebar( 'product_sort_bar' ) || is_ic_ajax() ) {
			...
			if ( !empty( $sidebar_content ) ) {
				...
				if ( !empty( $reset_url ) ) {
					echo '<div class="reset-filters"><a href="' . $reset_url . '">' . __( 'Reset Filters', 'post-type-x' ) . '</a></div>';
				}
			}
			$is_filter_bar = false;
			unset( $is_filter_bar );
...
function show_default_product_sort_bar( $archive_template, $multiple_settings = null ) {
	if ( get_option( 'old_sort_bar' ) == 1 ) {
		...
		if ( $show == 0 ) {
			...
			echo sprintf( __( 'Add widgets to the filters bar %snow%s or %sdismiss this notice%s.', 'post-type-x' ), '<a href="' . $customize_url . '">', '</a>', '<a class="dismiss-empty-bar" href="#">', '</a>' );
			echo '</div>';
			echo '</div>';
			$is_filter_bar	 = false;
			unset( $is_filter_bar );
