...
function create_sample_product() {
	...
	if ( (current_user_can( 'publish_products' ) || current_user_can( 'administrator' )) && (((!is_advanced_mode_forced() || is_ic_shortcode_integration()) && empty( $sample_id )) || isset( $_GET[ 'create_sample_product_page' ] )) ) {
		...
		$product_field[ 'excerpt' ]				 = '<p>' . __( 'Welcome on product test page. This is short description. It should show up on the left (for plain product page template) or right (for Formatted product page template) of the product image and below product name.', 'post-type-x' ) . '</p>';
		$product_field[ 'excerpt' ]				 = '<p>' . __( 'You can change the product page template in catalog settings.', 'post-type-x' ) . '</p>';
		$product_field[ 'excerpt' ]				 .= '<p><strong>' . __( 'Please read this page carefully to fully understand all product page elements.', 'post-type-x' ) . '</strong></p>';
...
function ic_sample_long_desc() {
	$long_desc	 = '<p>' . __( 'This section is product long description. It should appear under the attributes table or in the description tab. Before that you should see the price, SKU and shipping options (all can be disabled). The attributes also can be disabled.', 'post-type-x' ) . '</p>';
	...
	if ( !is_ic_shortcode_integration() ) {
		...
		$long_desc	 .= '<p>' . sprintf( __( 'Currently %s shortcode is being used on main product listing.', 'post-type-x' ), '[show_product_catalog]' ) . '</p>';
		...
		if ( !is_advanced_mode_forced( false ) ) {
			...
			$long_desc .= '<p>' . sprintf( __( 'To proceed with such test remove the %s shortcode from your main product listing page and see how the catalog pages look like without it.', 'post-type-x' ), '[show_product_catalog]' ) . '</p>';
		}
	}
	return $long_desc;
}
