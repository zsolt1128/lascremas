...
class ic_catalog_notices extends ic_activation_wizard {
	...
	function listing_page_info( $post ) {
		...
		if ( $listing_id == $post->ID ) {
			implecode_info( sprintf( __( 'This page is defined as main catalog listing. You can change this in %scatalog settings%s.', 'post-type-x' ), '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php' ) . '">', '</a>' ) );
			if ( !is_ic_shortcode_integration() ) {
				echo '<div></div>';
				implecode_info( sprintf( __( 'If you have any problems with your catalog display, please add the %s shortcode to the page content and save.', 'post-type-x' ), '[show_product_catalog]' ) );
			}
		}
	}
...
//$integration_type	 = get_integration_type();
		...
		if ( is_integration_mode_selected() && get_integration_type() == 'simple' ) {
			...
			<div class="notice notice-updated is-dismissible ic-notice" data-ic_dismissible="notice-ic-catalog-welcome">
				<div class="squeezer">
					...
					<h4><?php echo __( 'To switch to Advanced Mode please add the [show_product_catalog] shortcode to your product listing page.', 'post-type-x' ); ?></h4>
					...
					<p class="submit">
						...
						<script>jQuery( ".add-catalog-shortcode" ).click( function ( event ) {
			                    ...
			                    jQuery.post( ajaxurl, data, function ( response ) {
			                        ...
			                        jQuery( ".add-catalog-shortcode" ).replaceWith( "<?php _e( 'The shortcode has been added successfully!', 'post-type-x' ) ?>" );
			                        jQuery( ".skip.button" ).remove();
			                    } );
			                } );</script>
						<a class="skip button" href="<?php echo admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=support' ) ?>"><?php _e( 'Plugin Support', 'post-type-x' ); ?></a>
						<a class="skip button" href="<?php echo esc_url( add_query_arg( 'hide_al_product_adder_support_check', 'true' ) ); ?>"><?php _e( 'I know, don\'t bug me', 'post-type-x' ); ?></a>
					</p>
				</div>
			...
			<div class="notice notice-success is-dismissible ic-notice" data-ic_dismissible="notice-ic-catalog-welcome">
				<div class="squeezer">
					...
					<?php sprintf( __( '<strong>Your theme does not declare %s support</strong> &#8211; please proceed to sample product page where automatic layout adjustment can be done.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME ) ?>
					<?php __( 'You can create your product listing page automatically or insert [show_product_catalog] shortcode on any of your existing pages.', 'post-type-x' ) ?>
					...
					?>
					<a class="button-primary" href="https://implecode.com/docs/ecommerce-product-catalog/getting-started/#cam=default-mode&key=getting-started"><?php _e( 'Getting Started Guide', 'post-type-x' ) ?></a>
					<?php
	...
	function woocommerce_notice() {
		...
		if ( $this->show_woocommerce_notice() ) {
			...
			<div class="notice notice-info is-dismissible ic-notice" data-ic_dismissible="notice-ic-woocommerce-compat">
				<h4><?php echo sprintf( __( 'Hey! It looks like you have %1$s installed. You can use %2$s with or without %1$s.', 'post-type-x' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?> <?php _e( 'See the usage scenarios below:', 'post-type-x' ) ?></h4>
				<ul style="list-style: initial;list-style-position: inside;">
					<li><?php echo sprintf( __( '%1$s and %2$s separately for different product catalogs', 'post-type-x' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?> - <?php _e( "doesn't need any specific configuration", 'post-type-x' ) ?></li>
					<li><?php echo sprintf( __( '%1$s as an alternative to %2$s', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'WooCommerce' ) ?> - <?php echo sprintf( __( 'go to %1$s %3$simport screen%4$s to transfer all %2$s products to %1$s and remove %2$s after that', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'WooCommerce', '<a href="' . admin_url( 'edit.php?post_type=al_product&page=product-settings.php&tab=product-settings&submenu=csv' ) . '">', '</a>' ) ?></li>
					<li><?php echo sprintf( __( '%1$s with %2$s design', 'post-type-x' ), 'WooCommerce', IC_CATALOG_PLUGIN_NAME ) ?> - <?php echo sprintf( __( 'install free %1$s plugin from %2$scatalog extensions menu%3$s', 'post-type-x' ), 'WooCommerce Catalog', '<a href="' . admin_url( 'edit.php?post_type=al_product&page=extensions.php&tab=product-extensions' ) . '">', '</a>' ) ?></li>
				</ul>
			</div>
			<?php
		}
	...
	static function main_listing_button() {
		...
		if ( !empty( $listing_url ) ) {
			...
			?>
			<a href="<?php echo $listing_url ?>" class="button-secondary"><?php _e( 'See Main Catalog Listing', 'post-type-x' ); ?></a>
			<?php
		}
	}
...
//$links[] = '<a href="https://implecode.com/wordpress/plugins/premium-support/#cam=catalog-settings-link&key=support-link" target="_blank">Premium Support</a>';
	...
	function review_notice() {
		...
		$text = apply_filters( 'ic_review_notice_text', sprintf( __( '%s is a free software. Would you mind taking <strong>5 seconds</strong> to <a target="_blank" href="%s">rate the plugin</a> for us please? Your comments <strong>help others know what to expect</strong> when they install %s.', 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://wordpress.org/support/plugin/' . IC_CATALOG_PLUGIN_SLUG . '/reviews/#bbp-forum-0', IC_CATALOG_PLUGIN_NAME ) )
		?>
		<div class="notice notice-warning implecode-review is-dismissible">
			...
			<p><a target="_blank" href="https://wordpress.org/support/view/plugin-reviews/ecommerce-product-catalog#postform" class="button-primary"><?php _e( 'Rate Now & Hide Forever', 'post-type-x' ); ?></a>
				<?php /* <a href="" class="button"><?php _e( 'Hide Forever', 'post-type-x' ); ?></a> */ ?>
		</div>
		<div class="update-nag implecode-review-thanks" style="display: none"><?php echo sprintf( __( 'Thank you for <a target="_blank" href="%s">your rating</a>! We appreciate your time and input.', 'post-type-x' ), 'https://wordpress.org/support/view/plugin-reviews/ecommerce-product-catalog#postform' ) ?> <span class="dashicons dashicons-yes"></span></div><?php
	}

	function translation_notice() {
		?>
		<div class="update-nag implecode-translate"><?php echo sprintf( __( "<strong>Psst, it's less than 1 minute</strong> to add some translations to %s collaborative <a target='_blank' href='%s'>translation project</a>", 'post-type-x' ), IC_CATALOG_PLUGIN_NAME, 'https://translate.wordpress.org/projects/wp-plugins/ecommerce-product-catalog', IC_CATALOG_PLUGIN_NAME ) ?> <span class="dashicons dashicons-no"></span></div><?php
	}
